...
async function addEmployee() {
  ...
  employee.isActive = getInput('Active employee? (yes/no): ', isBooleanInputValid, (i) => (i.toLowerCase() === 'yes'));

  // Before calling getInput here, we call isIntegerValid(),
  // which returns a function (pointer) to a validator
...
function searchById() {
  ...
  // Shorter version
  // single param doesn't need(), slingle line doesn't require {}
  // and returns result of single operation
  // const result = employees.find(e => e.id === id);
...
Promise.all([getAllEmployees(), getCurrencyConversionData()])
  .then((results) => {
    [employees, currenceData] = results;
    // employees = results[0];
